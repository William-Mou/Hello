...
contract Transaction {
    ...
    uint expire = 10;
    uint working_hours;
    uint ETH_to_TWD_100 = 300000000000000;
    bool ended = false;
    bool usaged = false;
    /*
    event Deposit(
    ...
    function Transaction(address new_employee, uint new_working_hours ,uint new_base_wage)payable public{
        ...
        if (working_hours >= 2) payables = (working_hours-2)*base_wage*266+2*233*base_wage;
    ...
    function collectMoney() public {
        ...
        if (msg.sender!=employee || ended == false || usaged == false) revert();
